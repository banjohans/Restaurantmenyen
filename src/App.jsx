...
function App() {
  ...
  return (
    <div className={appStyles.dishCard}>
      <h1 className={appStyles.menuCard}>Bella Voce</h1>
      <h4 className={appStyles.ingress}>à la carte menu:</h4>
      {RestaurantMeny.map((meny) => {
        return <Menu key={meny.id} data={meny} />;
      })}
    </div>
  );
}
